...
const EXEC_TOOL_DEF = {
  ...
  description: "Execute a command on this machine (POSIX shell: macOS/Linux). Provide a shell command string in 'command'. For large/binary outputs, write files to $ECLIA_ARTIFACT_DIR (provided automatically). Files created there will be returned as artifacts. Each artifact includes: path (repo-relative), uri (eclia://artifact/...), and ref (<eclia://artifact/...>) for copy/paste referencing. Returns stdout/stderr/exitCode.",
  inputSchema: {
    type: "object",
    properties: {
      command: {
        type: "string",
        description:
          "Shell command string (pipes/redirection). Example: 'ls -la | sed -n 1,50p'."
      },
      cwd: {
        type: "string",
